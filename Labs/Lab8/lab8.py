...
def viewAndGetOutliers(df, colName, threshold, plt):
    ...
    dfSorted = dfSub.sort_values([colName[1]], ascending=[True])
    print("\nDataFrame rows containing outliers for " + colName + ":")
    print(dfSorted)
    return rowIndices

THRESHOLD_Z =2.33
col_used = ['MomAge', 'MomMarital', 'numlive', 'dobmm', 'gestation', 'weight']
priceOutlierRows = viewAndGetOutliers(df, col_used, THRESHOLD_Z, plt)
